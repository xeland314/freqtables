...
class TestTablasDeFrecuencias(unittest.TestCase):
    ...
    def test03_assert_columna_de_variables(self):
        x = ['A', 'B', 'C']
        self.assertListEqual(self.tabla1.variables, x)

    def test04_assert_columna_de_frecuencias(self):
        f = [3, 6, 1]
        self.assertListEqual(self.tabla1.frecuencias, f)

    def test05_assert_columna_de_f_relativas(self):
        fr = [0.3, 0.6, 0.1]
        self.assertListEqual(self.tabla1.frecuencias_relativas, fr)

    def test06_assert_columna_de_f_acumuludas(self):
        F = [3, 9, 10]
        self.assertListEqual(self.tabla1.frecuencias_acumuladas, F)

    def test07_assert_columna_de_f_r_acumuladas(self):
        Fr = [0.3, 0.9, 1.0]
        self.assertListEqual(self.tabla1.frecuencias_relativas_acumuladas, Fr)
